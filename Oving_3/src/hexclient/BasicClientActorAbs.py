    ...
    def play_tournament(self):
        ...
        while True:
            ...
            if state == 'Series start':
                unique_player_id = eval(self.ssl_sock.recv(1024).decode('utf8'))
                player_id_map = eval(self.ssl_sock.recv(1024).decode('utf8'))
            ...
            # We received an illegal action message
                ...
                self.handle_illegal_action(eval(state), eval(illegal_action))
                break

            # We received a request for a new move
            else:
                usr_move = str(self.handle_get_action(eval(state)))
